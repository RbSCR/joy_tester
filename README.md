...
More specifically, it displays the messages of the `sensor_msgs/Joy` and `sensor_msgs/JoyFeedback` topics in a more user-friendly format.
...
By default it subscribes to the `/joy` and `/joy/set_feedback` topic, but this can be remapped as with any other ROS node (e.g. with `--ros-args -r joy:=other_joy`).
